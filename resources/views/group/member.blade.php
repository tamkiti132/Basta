<x-app-layout>
      ...
      <div class="flex">
        ...
        <div>
          ...
          <div class="flex flex-col">
            ...
            <p class="mt-3 text-sm sm:mt-0 sm:ml-28">{{ $count_all_memos_data }}<span class="ml-3">投稿</span></p>
          </div>
        </div>
      </div>
      {{-- 右側 --}}
          ...
          <x-dropdown align="right" width="48">
            ...
            <x-slot name="content">
              <div class="flex flex-col px-4 text-gray-800">
                <button x-on:click="modal_report_user = true" class="block w-full p-2 text-left hover:bg-slate-100">
                  ユーザーを通報
                </button>
                <form method="POST" action="{{ route('group.member.destroy', ['id' => $user_data['id'] ]) }}">
                  @csrf
                  <button type="submit" class="block w-full p-2 text-left hover:bg-slate-100"
                    onclick="return confirm('本当に削除しますか？');">ユーザーを削除</button>
                </form>
                <form method="POST" action="{{ route('group.member.suspend', ['id' => $user_data['id'] ]) }}">
                  @csrf
                  <button type="submit" class="block w-full p-2 text-left hover:bg-slate-100"
  ...
  <div class="pt-12 mx-auto max-w-7xl sm:px-6 lg:px-8">
    <form method="get" action="{{ route('group.member.show', ['id' => $user_data['id']]) }}" class="text-left">
                ...
                <div class="relative px-4 pt-8 pb-8 bg-gray-100 bg-opacity-75 shadow-md sm:px-8 rounded-2xl">
                  <div class="grid gap-10 sm:grid-cols-7 sm:gap-0">
                    ...
                    <div class="sm:col-span-3">
                      ...
                      {{-- メモタイトル --}}
                      <div class="mt-5 ml-3 leading-none y-4">
                        <button class="self-center font-bold text-gray-700 break-all sm:text-xl title-font"
                          onclick="location.href='{{ route('group.memo_show.show',['id'=>$memo_data['id']]) }}' ">{{
                          $memo_data->title }}
                        </button>
                      </div>
